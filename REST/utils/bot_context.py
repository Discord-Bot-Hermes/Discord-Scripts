        ...
        if hasattr(self.author, 'target_user_id') and self.author.target_user_id:
            try:
                ...
                if user:
                    ...
                    print(f"Sent message to user [Server name: {user.display_name}] & [Discord name: {user.name}] through respond from {getattr(self.author, 'display_name')}#{getattr(self.author, 'discriminator')}")
                else:
                    print(f"Could not find user with ID {self.author.target_user_id}")
            except Exception as e:
                print(f"Error sending message to user {self.author.target_user_id}: {str(e)}")
    ...
    def __init__(self, user):
        ...
        try:
            ...
            if 'access_roles' not in settings:
                error_msg = "No access_roles found in settings.json"
            ...
            if admin_role_id is None:
                error_msg = "Admin role not found in settings.json access_roles"
                self.logger.error(error_msg)
                raise RuntimeError(error_msg)
        ...
        if self.target_user_id:
            try:
                ...
                if user:
                    ...
                    print(f"Sent DM to user {self.target_user_id}: {args}, {kwargs}")
                    return real_message
                else:
                    print(f"Could not find user with ID {self.target_user_id}")
            except Exception as e:
                print(f"Error sending DM to user {self.target_user_id}: {str(e)}")

        # Fall back to mock behavior if we can't send a real message
        print(f"Mock DM would send: {args}, {kwargs}")
        return MockMessage()
...
class MockMessage:
    ...
    async def edit(self, *args, **kwargs):
        ...
        print(f"Mock message would edit: {args}, {kwargs}")
        return self
