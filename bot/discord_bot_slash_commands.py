    ...
    match status.lower():
        case "start":
            if _verify_author_roles(ctx.author):
                update_dm_accept_status(group_id)
                await ctx.respond(
                    f"{ctx.author.mention}, accepting messages in DM for the group {group_id}."
                )
            else:
        ...
        case "stop":
            if _verify_author_roles(ctx.author):
                await ctx.respond(
                    f"{ctx.author.mention}, messages in DM are no longer accepted for the group {group_id}."
                )
                ...
                embed.add_field(
                    name="Students List",
                    inline=False,
                    value=prepare_group_list_for_embed(group_id),
                )
                ...
                print(f"{group_id}---------------------")
                attendance_cleanup(group_id=group_id)
            else:
                await ctx.respond(bot_data.PERMISSION_DENIED)
        case _:
            await ctx.respond(
...
async def tutor_session_feedback(
    ...
    if _verify_author_roles(ctx.author):
        ...
        await ctx.respond(f"Feedback was created in channel {channel}, the timer is set to: " + str(view.timeout) + " seconds.", )
    else:
        print(bot_data.PERMISSION_DENIED)
        await ctx.respond(bot_data.PERMISSION_DENIED)
